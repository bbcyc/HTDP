...
; S-expr Sy Sy -> S-expr
; returns an S-expr where every instance of a given
; symbol is replaced with the other given symbol
(check-expect (substitute 'world 'world 'hello) 'hello)
(check-expect (substitute '(world hello) 'hello 'world) '(world world))
(check-expect (substitute '(((world) hello) hello) 'world 'hello) '(((hello) hello) hello))
(define (substitute s old new)
  (local
    (; substitute symbol in atom
     (define (sub-atom at)
       (cond
         [(string? at) at]
         [(number? at) at]
