...
(check-expect (to-image (cons 10 (cons 20 '())))
              (place-images
               (list SHOT SHOT)
               (list (make-posn XSHOTS 10)
                     (make-posn XSHOTS 20))
               BACKGROUND))
(define (to-image w)
  (cond
...
; ShotWorld -> ShotWorld
; Removes shots with negative values
(check-expect (tockPos (cons 10 (cons 5 '()))) (cons 9 (cons 4 '())))
(check-expect (tockPos (cons 0 (cons 5 '()))) (cons 4 '()))
(define (tockPos w)
  (cond
...
(check-expect (tock (cons 10 (cons 5 '()))) (cons 9 (cons 4 '())))
(define (tock w)
  (cond
    [(empty? w) '()]
    [else (tockPos w)]))
...
(check-expect (keyh (cons 10 '()) "left") (cons 10 '()))
(check-expect (keyh (cons 10 '()) " ") (cons HEIGHT (cons 10 '())))
(define (keyh w ke)
  (cond
