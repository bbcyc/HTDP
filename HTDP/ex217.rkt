...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname ex217) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (clock-tick-handler w)
  (make-worm (worm-dir w) (cons (create-segment (worm-dir w) (first (worm-body w))) (drop-last (worm-body w)))))
...
(define (end? w)
  (cond
    [(or (> 0 (posn-y (first (worm-body w))))
         ...
         (< SEGHEIGHT (posn-y (first (worm-body w))))) #true]
    [else #false]))
